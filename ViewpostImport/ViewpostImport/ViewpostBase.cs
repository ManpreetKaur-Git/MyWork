...
namespace ViewpostImport
{
    public partial class ViewpostBase
    {
        ...
        public static void GetVPData()
        {
            ...
            try
            {
                ...
                using (var Client3 = new HttpClient())
                {
                    ...
                    var buffer1 = System.Text.Encoding.UTF8.GetBytes(xmlElem.LastNode.ToString());
                    var byteContent1 = new ByteArrayContent(buffer1);
                    byteContent1.Headers.ContentType = new MediaTypeHeaderValue("application/xml");
                    Client3.DefaultRequestHeaders.Add("Authorization", sVPAuthToken);
                    var response3 = Client3.PostAsync(url1, byteContent1).Result;
                    if (response3.IsSuccessStatusCode)
                    {
                        Thread.Sleep(4000);
                        string URL2 = response3.Headers.Location.AbsoluteUri;
                        HttpClient client = new HttpClient();
                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/xml"));
                        client.DefaultRequestHeaders.Add("Authorization", sVPAuthToken);
                        HttpResponseMessage response2 = null;
                        response2 = client.GetAsync(URL2).Result;
                        if (response2.IsSuccessStatusCode)
                        {
                            Thread.Sleep(3000);
                            string URL3 = response2.Headers.Location.AbsoluteUri;
                            HttpClient client1 = new HttpClient();
                            client1.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/xml"));
                            client1.DefaultRequestHeaders.Add("Authorization", sVPAuthToken);
                            HttpResponseMessage response1 = null;
                            response1 = client1.GetAsync(URL3).Result;
                            if (response1.IsSuccessStatusCode)
                            {
                                Thread.Sleep(3000);
                                string dataObjects = response1.Content.ReadAsStringAsync().Result;
                            ...
                            {
                                ExitCode = JobStatus.JOB_FAILED;
                                InfoMessage = FatalError = "Error:Response not found  while fetching Viewpost data VP's Bulk API.Reason:  " + response1.ReasonPhrase;
                            }
                        }
                        else
                        {
                            ExitCode = JobStatus.JOB_FAILED;
                            InfoMessage = FatalError = "Error:Response not found  while fetching Viewpost data VP's Bulk API.Reason:  " + response2.ReasonPhrase;
                        }
                    }
                    else
                    {
                        ExitCode = JobStatus.JOB_FAILED;
                        InfoMessage = FatalError = "Error:Response not found  while fetching Viewpost data VP's Bulk API.Reason:  " + response3.ReasonPhrase;
        ...
        public static void UpdatePaymentResp(Dictionary<string, string> Vendorpayment, int Count)
        {
            ...
            if (Count == 0)
            {
                sSql.Append("INSERT INTO VP_CHECKPOINT (CHECK_POINT,DTTM_RCD_SENT) VALUES( '" + Vendorpayment["checkpointToken"] + "' , '" + DateTime.Now.ToString("yyyyMMddhhmmss") + "' )");
            ...
            iTransID = Convert.ToInt32(DbFactory.ExecuteScalar(ConnectionString, sSql.ToString()));
        ...
        public static int GetNextUID(string tablename)
        {
            ...
            using (DbReader reader = DbFactory.ExecuteReader(TaskDatabase, sqlQuery.ToString()))
            {
                if (reader.Read())
                {
                    rowID = Convert.ToInt32(reader.GetValue("NEXT_ID"));
                    if (rowID == 0)
                    {
                        rowID += 1;
                        nextUD = rowID + 1;
